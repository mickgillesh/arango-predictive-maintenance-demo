...
deploy:
	gcloud run deploy $(SERVICE) \
	  ...
	  --memory 1Gi \
	  --set-env-vars "ARANGO_URL=$(ARANGO_URL),ARANGO_DB=$(ARANGO_DB),ARANGO_USER=$(ARANGO_USER),ARANGO_PASSWORD=$(ARANGO_PASSWORD),OPENAI_API_KEY=$(OPENAI_API_KEY),GOOGLE_CLIENT_ID=$(GOOGLE_CLIENT_ID),GOOGLE_CLIENT_SECRET=$(GOOGLE_CLIENT_SECRET),GOOGLE_REDIRECT_URI=$(GOOGLE_REDIRECT_URI)"
